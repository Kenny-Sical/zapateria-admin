document.addEventListener('DOMContentLoaded', function () {
    ...
    function buildExcelTable(colors, sizes) {
        ...
        const existingInv = window.existingInventory || {};
        ...
        colors.forEach(color => {
            ...
            sizes.forEach(size => {
                // Recuperar valor existente si lo hay
                let val = 0;
                if (existingInv[color.id] !== undefined && existingInv[color.id][size.id] !== undefined) {
                    val = existingInv[color.id][size.id];
                }
