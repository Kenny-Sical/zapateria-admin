document.addEventListener("DOMContentLoaded", function () {
    ...
    btnNext.addEventListener("click", function () {
        ...
        const selectedColors = Array.from(colorsSelect.selectedOptions).map(
            (opt) => ({ id: opt.value, name: opt.text }),
        );
        ...
        if (selectedColors.length === 0 || selectedSizes.length === 0) {
            Swal.fire(
                "Faltan Datos",
                "Debes seleccionar al menos un color y una talla para poder configurar el stock.",
                "warning",
            );
            return;
        }
    ...
    btnSubmit.addEventListener("click", function (e) {
        ...
        if (!hasQuantity) {
            ...
            Swal.fire(
                "Faltan Datos",
                "Debes ingresar una cantidad mayor a 0 en al menos una combinación de talla y color.",
                "warning",
            );
            return;
        }
